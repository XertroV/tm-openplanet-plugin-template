...
class HookHelper {
    ...
    void FindPattern() {
        ...
        trace("Found pattern for " + functionName + ": " + Text::FormatPointer(patternPtr));
    }
    ...
    bool Apply() {
        ...
        @hookInfo = Dev::Hook(patternPtr + offset, padding, functionName, Dev::PushRegisters::Basic);
        return true;
    }
    ...
    void UnapplySoon() {
        Meta::StartWithRunContext(Meta::RunContext::Main, CoroutineFunc(_UnapplyCoroF));
    }
    void _UnapplyCoroF() {
        trace('unapplying hook for ' + functionName + ' now');
        Unapply();
        trace('unapplied');
    }
...
class FunctionHookHelper : HookHelper {
    ...
    bool Apply() override {
        ...
        if (cavePtr + 5 + newCallRelOffset != patternPtr + offset + 5 + origCallRelOffset) {
            warn("FunctionHookHelper: bad new call offset. cavePtr: " + cavePtr + ", newCallRelOffset: " + newCallRelOffset + ", origCallRelOffset: " + origCallRelOffset + ", functionPtr: " + functionPtr + ", patternPtr: " + patternPtr + ", offset: " + offset);
            HookHelper::Unapply();
            return false;
        }
    ...
    bool Unapply() override {
        ...
        if (functionPtr == 0 || cavePtr == 0) {
            warn("FunctionHookHelper: bad function ptr or cave ptr. function ptr: " + functionPtr + ", cave ptr: " + cavePtr + ". Failed to unapply hook for " + functionName);
            return false;
        }
...
// this can be called on plugin unload or if you want to unhook everything, but the destructor of HookHelper will also handle it
